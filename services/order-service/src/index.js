...
const kafka = new Kafka({ clientId: 'order-service', brokers: [kafkaBroker] });
...
// Creates an order and publishes order.created to both Kafka and RabbitMQ.
app.post('/orders', async (req, res) => {
  ...
  res.status(201).json({ orderId, published: true });
});
